...
__export(exports, {
  default: () => BetterReadingPlugin
});
var import_obsidian2 = __toModule(require("obsidian"));

// src/betterReadingWidget.ts
...
// src/betterReadingWidget.ts
function betterReadingExtension(app2, plugin) {
  return import_view.ViewPlugin.fromClass(class {
    constructor(view) {
      this.view = view;
      this.betterReadingDecorations = import_view.Decoration.none;
    ...
    getDeco(view) {
      var _a;
      let { state } = view, betterReadingDecos = [];
      if (!(plugin == null ? void 0 : plugin.settings.betterReadingMode)) {
        return {
          textDeco: import_view.Decoration.set(betterReadingDecos)
        };
      }
      for (let part of view.visibleRanges) {
        let betterReadingCursor;
        try {
        ...
        while (!betterReadingCursor.next().done) {
          ...
          let syntaxNode = (0, import_language.syntaxTree)(view.state).resolveInner(linePos + 1), nodeProps = syntaxNode.type.prop(import_language.tokenClassNodeProp), excludedSection = ["hmd-codeblock", "hmd-frontmatter", "cm-inline-code", "comment", "header"].find((token) => nodeProps == null ? void 0 : nodeProps.split(" ").includes(token));
          if (excludedSection)
            continue;
          const markDeco = import_view.Decoration.mark({ class: "cm-better-reading" });
          ...
          if (wordLength <= 3) {
            ...
            betterReadingDecos.push(markDeco.range(from, from + Math.ceil(wordLength * 0.5)));
          }
        }
      }
      return {
        textDeco: import_view.Decoration.set(betterReadingDecos.sort((a, b) => a.from - b.from))
      };
    }
  }, {
    provide: (plugin2) => [
      import_view.EditorView.decorations.of((v) => {
        var _a;
        return ((_a = v.plugin(plugin2)) == null ? void 0 : _a.betterReadingDecorations) || import_view.Decoration.none;
      })
    ]
  });
}
